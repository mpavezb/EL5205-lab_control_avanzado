...
typedef RT_MODEL_Planilla_3DOF_Crane RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Planilla_3DOF_Crane(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Planilla_3DOF_Crane_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Planilla_3DOF_Crane_DWork.HILReadTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Planilla_3DOF_Crane -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        fflush(stdout);
        result = hil_task_start(Planilla_3DOF_Crane_DWork.HILReadTimebase_Task,
                                (t_clock)
          Planilla_3DOF_Crane_P.HILReadTimebase_Clock, 1000.0, -1)
          ;
        if (result == 0) {
