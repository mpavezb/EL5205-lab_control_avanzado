...
typedef RT_MODEL_q_3d_crane_cal RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_3d_crane_cal(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_3d_crane_cal_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(q_3d_crane_cal_DWork.HILReadTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://q_3d_crane_cal:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_3d_crane_cal -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        fflush(stdout);
        result = hil_task_start(q_3d_crane_cal_DWork.HILReadTimebase_Task,
          (t_clock) q_3d_crane_cal_P.HILReadTimebase_Clock, 1000.0, -1)
          ;
        if (result == 0) {
